...
- 各機種の内蔵 B-CAS カードリーダーを利用するためのドライバを追加
  - 同梱の `WinSCard.dll` を利用するアプリケーションの実行ファイルと同じフォルダに配置すると、外付けカードリーダーの代わりに内蔵カードリーダーを使えるようになる
  - 他の `WinSCard.dll` や外付けカードリーダーとは併用できないため注意
...
> [!WARNING]
> **他の `WinSCard.dll` 実装と同様に、外付けカードリーダーと内蔵カードリーダーを併用することはできません。**  
> 外付けカードリーダーの使用に切り替えたい場合は、配置した `WinSCard.dll` を削除するかリネームしてください。  
...
> **「設定」→「TSプロセッサー」→「デフォルトフィルター」のセレクトボックスで利用する内蔵カードリーダー名を選択し、TVTest を再起動すると、内蔵カードリーダーに挿入された B-CAS カードを使ってスクランブル解除を行えます。**  
